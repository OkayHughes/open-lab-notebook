...
\section{Oksana's notes}
For the purposes of disambiguation, I'll stick with calling pseudodensity $\mu$ and refer to the pressure correction factor ($\mu$ in Oksana's notes) as $\psi.$
...
\begin{align*}
    ...
    &= -g\rho r_s \hat{r}^2
\end{align*}
is that $g = g_0 \hat{r}^{-2}.$ This implies that the actual pressure \textit{value} per gridpoint in the deep atmosphere would be essentially the same, but it would represent a fundamentally different quantity.
...
\section{Following Quasi-hamiltonian derivation in Tea20}
...
\textcolor{red}{Shallow atmosphere mass integral:
\begin{align*}
...
which demonstrates the shallow atmosphere identity from the paper. Let's do this for the deep atmosphere in a naive way:
\begin{align*}
...
\end{align*}
and we do some algebraic shenanigans and discard second-order terms as usual:
\begin{align*}
